...
class Download(threading.Thread):
	...
	complete = False
	progress = None
	def __init__(self,url,options = None,callback = None):
		if options == None:
			self.options = config.Pconfig()
		else:
	...
	def run(self):
		try:
			output_file = self.url.rsplit("/", 1)[1]   # basename of the url
			...
			for i in range(self.options.num_connections):
				...
				current_thread = FetchData(i, self.url, output_file, state_file,
